...
class Config:
    ...
    ONLINE: bool = True  # ✅ enable online mode
...
def load_prices(cfg: Config) -> pd.DataFrame:
    """Load daily adjusted close prices from CSV or yfinance."""
    if cfg.data_dir:
        frames = []
...
def momentum_score(prices_m: pd.DataFrame, lookbacks: List[int]) -> pd.DataFrame:
    """Average of L-month returns excluding last 1 month."""
...
def backtest(prices: pd.DataFrame, cfg: Config) -> Dict[str, pd.DataFrame]:
    """Run strategy and return performance data."""
    ...
    rets_period = prices_f.pct_change().fillna(0.0)
    port_rets_period = (weights * rets_period).sum(axis=1)
...
def main():
    cfg = Config(
        ...
        freq="M",  # 'M' for monthly or 'W-FRI' for weekly
        ...
        data_dir=None,
        ONLINE=True  # ✅ using live yfinance data
    )
